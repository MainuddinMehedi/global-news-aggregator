...
export default function FindingsFilter({ currentSource, currentSort }: { currentSource: string, currentSort: string }) {
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 py-6 border-y border-secondary/50">
      ...
      <div className="flex items-center gap-4">
        <span className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/60">Sequence</span>
        <Select value={currentSort} onValueChange={(v) => updateParam('sort', v)}>
          ...
          <SelectContent className="rounded-xl border-secondary shadow-2xl backdrop-blur-xl">
            {SORTS.map((s) => (
              <SelectItem key={s.value} value={s.value} className="text-xs font-bold uppercase tracking-wider py-3">
                {s.label}
              </SelectItem>
            ))}
          </SelectContent>
