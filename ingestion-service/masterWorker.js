...
import { processNotificationDigests } from "./notifications/topicsDigestWorker.js";
import { processSystemNewsDigests } from "./notifications/systemDigestWorker.js";
...
async function registerWorker(
  ...
  await boss.work(
    queueName,
    { concurrency: 1, batchSize: 1, ...options },
    async (jobs) => {
...
async function runMasterWorker() {
  ...
  console.log(
    "🚀 [Master Worker] Started, initializing schedules and queues...",
  );

  // Factor out worker definitions using registerWorker utility
  await registerWorker(boss, "ingest-queue", "*/30 * * * *", () =>
    runIngestionPipeline(),
  );
  await registerWorker(boss, "cluster-queue", "45 * * * *", () =>
    runClusteringLogic(),
  );
  await registerWorker(boss, "backlog-queue", "0 3 * * *", () =>
    processBacklogLogic(),
  );
  await registerWorker(
    ...
    "topics-queue",
    "15 */2 * * *",
    (job) => scanTopicsLogic(job.data?.topicId),
    { concurrency: 4 },
  );
  await registerWorker(boss, "notification-delivery", "* * * * *", () =>
    processDeliveryBatch(),
  );
  await registerWorker(boss, "notification-digest", "0 * * * *", () =>
    processNotificationDigests(),
  ); // Runs every hour
  await registerWorker(boss, "system-news-digest", "0 * * * *", () =>
    processSystemNewsDigests(),
  ); // Runs every hour
  await registerWorker(boss, "health-monitor", "*/15 * * * *", () =>
    runHealthMonitor(),
  );
  await registerWorker(boss, "notification-retention", "0 4 * * *", () =>
    cleanupNotifications(),
  );
  await registerWorker(boss, "anonymous-chat-cleanup", "0 4 * * *", () =>
    cleanupAnonymousChats(),
  );
